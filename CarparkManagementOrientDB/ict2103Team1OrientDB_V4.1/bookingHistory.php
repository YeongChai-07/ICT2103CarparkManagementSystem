                            ...
                            <tbody>
                                <?php
								 ...
								 if ($successfulConnection){
									...
									$query = "SELECT booking FROM user WHERE userID = {$userID}";
									...
									foreach($record[0]->getOData()['booking'] as $booking){
										// must add one day to starting date as the codes print out the database early by one day
										echo "<tr><td>" . $booking['location'] . "</td><td>" . $booking['lotNumber'] . "</td><td>" . $booking['totalCost']."</td><td>" . $booking['startDate']->add(new DateInterval('P1D'))->format('Y-m-d') . "</td></tr>";
									}
								}
								$client->dbClose();
                                ?>
